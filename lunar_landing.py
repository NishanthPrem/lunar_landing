...
scores_100_eps = deque(maxlen=100)

for episode in range(1, number_eps + 1):
  state, _ = env.reset()
  score = 0
  for t in range(max_timesteps_per_ep):
    ...
    if done:
      break
  scores_100_eps.append(score)
  epsilon = max(epsilon_end, epsilon_decay * epsilon)
  print('\rEpisode {}\tAverage Score: {:.2f}'.format(episode, np.mean(scores_100_eps)), end = "")
  if episode % 100 == 0:
    print('\rEpisode {}\tAverage Score: {:.2f}'.format(episode, np.mean(scores_100_eps)))
  if np.mean(scores_100_eps) >= 200.0:
    print('\nEnvironment solved in {:d} episodes!\tAverage Score: {:.2f}'.format(episode - 100, np.mean(scores_100_eps)))
